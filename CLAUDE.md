...
The system uses a multi-agent approach with 9 specialized AI agents that work together through a LangGraph workflow:
...
The agents collaborate through a LangGraph state machine (`src/workflow/graph.py`) that manages the workflow between different phases of the writing process.
...
### Workflow System
Based on LangGraph with specialized nodes for each agent (`src/workflow/nodes.py`):
- Defines conditional edges between different writing phases
- Manages state using GraphState
- Tracks chapter progress and story metrics
